...
const writeOperation = text => {
    ...
    if(screenOperation.textContent.length > 24) {
...
const writeResult = () => {
    if(isNaN(lastValue()) && lastValue()!== ')') screenOperation.textContent.substring(screenOperation.textContent.length-1);
    ...
    if(screenResult.textContent.length > 9) {
        ...
        screenResult.style.marginTop = '1rem';
    }
}
...
buttons.addEventListener('click', ev => {
    if (ev.target.textContent !== '') {
        switch(ev.target.textContent) {
            ...
            default: writeOperation(ev.target.textContent); break;
